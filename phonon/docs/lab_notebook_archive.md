...
### ⚙️⚙️ PRODUCTION (`src/quatrex`, distributed/MPI) node usage — USE THE NODE, single-threaded BLAS
This is **our private 256-core node — no other users**. So actually *use* it: scale with **MPI
ranks**, one core per rank. The harness is `phonon/scripts/prod/{build_inputs,write_config,run}.py`.
- **THREADED BLAS IS A TRAP.** The production phph parallelizes over MPI ranks, NOT a thread
  pool. Setting `OPENBLAS_NUM_THREADS>1` makes each rank spawn ~257 threads (numba+BLAS) →
  catastrophic oversubscription (measured **load 1163** on 256 cores, every run hangs). The
  batched ring-contract matmuls are batch-of-small and don't thread efficiently anyway. **Always
  `OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1`**, and put
  the parallelism in `mpirun -np N --bind-to core --map-by core`.
- **Scaling axes (profiled: the 3-phonon bubble is ~99.8% of an SCBA iteration).** The bubble
  scales near-ideally on the **stack/energy axis** (CNT L=2: 16×@16 ranks, then a ~12s FFT/
  transpose floor; bigger nfreq/devices saturate later). The **q axis** (`q_comm_size`, films,
  k>1) and the **block axis** (`block_comm_size`, CNT/wires, k==1) compose on top → use
  `stack × q` (film) or `stack × block` (CNT) to fill more of the node on the bigger benchmark
  problems. RGF Dyson solve + OBC are <0.1% — don't bother optimizing them.
- **ALWAYS cleanup + check before/after a run** (a killed `mpirun` leaves its ranks spinning
  257 threads each → silent node saturation that wrecks every subsequent run):
  ```
  pkill -9 -f "run.py" ; pkill -9 -f "mpirun" ; pkill -9 -f "prterun"   # kill stale (match the CMDLINE "python -u run.py", NOT a path)
  pgrep -u $(id -u) -f "run.py|mpirun|prterun" | wc -l                  # must be 0 before launching
  uptime                                                                # load should be ~O(your ranks), not 100s
  ```
  Budget runs against the *free* cores (`load` + your planned ranks ≲ 256).
- **`max_batch_size` for films (k>1, fine grids):** the RGF batches the energy when
  `get_batches(nfreq, max_batch_size)` returns >1 batch (≈ nfreq ≳ 150). Its `return_current`
  lead-current path (`rgf.py` ~l.409) then matmuls the full-nfreq OBC against a per-batch G and
  raises a shape error (this is what looked like a "small-eta Dyson NaN" — it is NOT a real
  instability; the RGF is correct unbatched). Fix: set `[phonon.solver] max_batch_size` ≥ nfreq
  (the prod `write_config.py` defaults it to 100000 → never batches). The `Inv` solver
  (`algorithm = "inv"`) handles batching correctly and is also a small-eta-robust alternative.
- **Convergence = a real fixed point, not best-iterate.** The anharmonic SCBA must converge the
  self-energy to self-consistency (Sigma residual small) AND conserve heat flow. Do NOT accept a
  non-converged Sigma at merely-okay conservation; if it oscillates, fix the mixing (Anderson /
  smaller linear), don't best-iterate a transient. The film (no soft mode) converges cleanly.
...
### F33 — d5a soft-mode SCBA: Kramers-Kronig instability, rescued by a self-consistent SCP tadpole
Tested whether the static loop/tadpole + the bubble's Kramers-Kronig real part change the d5a SiNW
soft-twist convergence (`scripts/verify/d5a_scp_{softmode,selfconsistent,fft_tadpole}.py`,
`plot_d5a_scp.py`; fig `document/fig/transport_sweeps/d5a_scp_convergence.pdf`). n_slabs=1, 300 K.
- **(b) Kramers-Kronig is the root of the non-convergence.** `build_retarded` (solver/retarded.py):
  `fft` = 1/2(Sig>-Sig<) + i/2 Hilbert(...) = the FULL causal Sigma^R (broadening + KK real shift);
  `half` = 1/2(Sig>-Sig<) only = broadening, DROPS the KK real shift. Full-KK `fft` does NOT converge
  (residual plateaus ~0.02 then RISES to ~2) -- the KK real-part renormalisation destabilises the soft
  twist. `half` converges cleanly (44 it, resid 9.5e-4, Ga/Gb 0.967) but is an approximation.
- **(a) tadpole must be SELF-CONSISTENT.** A fixed (one-shot harmonic) wire <uu> is dominated by the soft
  twist's divergent <u^2>~1/omega^2 (=39.9 amu*A^2/dir, 250x bulk Si) -> runaway tadpole
  ||Sigma_static||~24000, residual spikes wildly, no convergence. The self-consistent <uu> (device G^<
  recomputed each iter; tadpole=True, NO static_uu, stage_loop_first, static_mixing 0.1) self-limits:
  as the tadpole stiffens the mode, <uu>~1/omega^2 drops, ||Sigma_static|| saturates at ~318.
- **KEY: the self-consistent tadpole RESCUES the rigorous full-KK calc.** `fft` + self-consistent tadpole
  CONVERGES (59 it, resid 8.9e-4, ||Sigma_static||=318) where `fft`-alone diverges. The static SCP
  stiffening (+318 to the dynamical matrix) lifts the soft twist enough that the full causal Sigma^R finds
  a stable fixed point. This is the SCP mechanism: pre-stiffen the soft mode so the bubble is stable.
- **Caveats:** heat-flow conservation stays ~2% throughout (soft-mode causality defect is SEPARATE, not
  fixed by the tadpole). Ga/Gb=1.034 (>1) for fft+tadpole is NOT a clean reduction -- the tadpole
  renormalises (stiffens) the bands vs a harmonic G_ball reference; at 2% conservation it's qualitative.
  The run's value is the convergence/stabilisation demonstration, not the transport number.
- **The LOOP (FC4) -- the stronger, rigorous SCP stiffening -- is untested on d5a:** its 84-atom cell
  gives a 30 GiB dense FC4 export (>8 GiB guard); needs the sparse FC4 exporter (or a CSLD-style direct
  loop estimate -- under investigation).

### F34 — d5a loop/tadpole/bubble + a FEASIBLE FC4 fit; the ultra-soft twist makes the static SE fragile
Following F33, investigated the full SCP (bubble + tadpole + loop) on d5a and how to get a usable FC4.
- **Feasible FC4 fit (NO new DFT):** the existing reap used ardr, which PRUNES the d5a FC4 to ZERO (the
  small-amplitude rattle's quartic signal is sub-threshold for ardr). **rfe (or ridge) recovers it**: on
  the SAME 44 structures, adding FC4 drops the test RMSE 7x (0.031 -> 0.0045, test~train, NOT overfit),
  max|FC4| 110 eV/A^4. So "feasible fitting" = use rfe, not ardr (`scripts/verify/d5a_fc4_fitting.py`).
  A naive CSLD/TDEP effective-FC2 refit (FC2-only to the rattle) FAILS (imaginary modes -- the rattle is
  a Taylor-fit set, not a thermal ensemble; needs proper SSCHA sampling).
- **Sparse FC4 / export:** hiphive's `get_fc_dict(order=4)` is sparse (557 clusters / 0.3 MB for d5a, vs
  84^4=30 GiB dense), so the loop Sigma_L=1/2 Phi4:<uu> (FC2-sized) is contractable on the fly. For now,
  made the reap's dense-FC4 guard configurable (`QUATREX_FC4_MAX_GIB`, default 8) and built the 30 GiB
  dense in RAM (503 GB node) to reuse the tested dense->compact exporter. d5a fc3.hdf5 now carries
  fc2/fc3/fc4 (rfe; ardr backup at fc3_ardr_backup.hdf5). Config `sinw100_d5a_vasp_sc4_fc4.yaml`.
- **Bubble converges with the rfe FCs** (`scripts/verify/d5a_loop_tadpole_bubble.py`, n_slabs=1, fft):
  conv in 33 it, conservation **1.09%** (vs ardr ~2% + non-convergence), Ga/Gb 1.003. So a LARGE part of
  d5a's soft-mode SCBA trouble was **FC-fit quality** (ardr's noisier FC3), not just KK/tadpole physics.
  (rfe and ardr give IDENTICAL FC2/dispersion -- soft twist 0.0075 THz either way; only FC3/FC4 differ.)
- **Tadpole + loop (self-consistent static SE) are NUMERICALLY FRAGILE on the ultra-soft twist.** The
  twist at 0.0075 THz (omega^2 ~ 6e-5 THz^2) makes `mean_displacement`'s Phi_eff^{-1} solve amplify by
  ~1/omega^2 ~ 1.7e4 -> the static SE overshoots to NaN and crashes the eigensolver (LinAlgError),
  regardless of loop_only/full_G propagator, staging, or static_mixing 0.05-0.1. The F33 tadpole
  converged (Sigma=318) only because the ardr FC3 happened to find a stable path; the rfe FC3 tips it
  over. The bubble (no such solve) is stable. **Fix direction:** regularise the mean_displacement solve
  near the soft eigenvalue (Tikhonov / pseudo-inverse cap), or proper mode-by-mode SSCHA self-consistency.
- **L=2 (n_slabs=2) with cutoffs:** diagonal cutoffs (s0/v0/g0) + larger eta converge (29 it) but
  UNPHYSICALLY (conservation 11-13%; diagonal-G catastrophic per F25). A physical L=2 needs the
  off-diagonal blocks -> too costly in the dense solver (RGF/distributed regime). Tadpole at L=2-diagonal
  adds Sigma_static=104 but doesn't fix conservation.
