...
1. Primitive cell (2-atom FCC): FC2 + FC3 from phono3py, no remapping.
   Requires reaps/si_primitive_work/fc3.hdf5 and reaps/si_primitive_work/phono3py_disp.yaml.
...
def load_primitive_cell_dfpt(work_dir):
    ...
    if not dfpt_h5.exists():
        raise FileNotFoundError(
            ...
            "  python -m phonon_inputs dfpt-sow  --config configs/si_primitive/dfpt.yaml\n"
            "  python -m phonon_inputs dfpt-run  --config configs/si_primitive/dfpt.yaml\n"
            "  python -m phonon_inputs dfpt-reap --config configs/si_primitive/dfpt.yaml"
        )
...
def load_primitive_cell(work_dir, fc3_subdir="reaps/si_primitive_work"):
    ...
    if not fc3_h5.exists():
        raise FileNotFoundError(
            ...
            "  python -m phonon_inputs fc3-sow  --config configs/si_primitive/prim.yaml\n"
            "  python -m phonon_inputs fc3-run  --config configs/si_primitive/prim.yaml\n"
            "  python -m phonon_inputs fc3-reap --config configs/si_primitive/prim.yaml"
        )
