...
from phonon_inputs.constants import CONVERSION, THZ_TO_RAD, HBAR_EV, KB_EV
...
# Use finer frequency grid for lifetime extraction
t0 = time.time()
...
# In our formulation: G^R = (w^2 + i*eta - H - Sigma^R)^{-1}
# The broadening of mode w_0 is: gamma = |Im[Sigma^R(w_0)]| / (2*w_0)
# Phonon lifetime: tau = 1 / (2*gamma) = w_0 / |Im[Sigma^R(w_0)]|
...
mfp_nm = tau_ps * 1e-12 * v_group_approx * 1e9  # in nm

print("\nExtracted phonon scattering parameters at 300K:")
print(f"{'Freq [THz]':>12} {'|Im Sigma^R| [(rad/s)^2]':>25} "
      f"{'gamma [THz]':>13} {'tau [ps]':>10} {'MFP [nm]':>10}")
print("-" * 75)
for iw in range(0, len(freqs_thz), 10):
    if freqs_thz[iw] > 0.5:
        print(f"{freqs_thz[iw]:>12.1f} {im_sigma_avg[iw]:>25.4e} "
...
for T in temperatures:
    ...
    res = anharmonic_transmission(
        ...
        eta_factor=0.5,
        temperature=float(T),
        max_scba_iter=1,  # Single iteration = Born approximation
        scba_tol=1e-10,
        mixing=1.0,
        fc3_mode="full",
        verbose=False,
    ...
    # Extract average scattering rate
    ...
    print(f"  Avg |Im Sigma^R|: {avg_scatt:.4e} (rad/s)^2")
...
# Known experimental/theoretical values for Si at 300K
...
d_slab = a_conv * 1e-10  # slab thickness in m
...
print(f"   at d={a_conv:.2f} A << MFP is firmly ballistic)")
...
# Add reference points from Esfarjani & Chen (2011) approximate values
ref_freqs = [1.0, 2.0, 3.0, 5.0, 7.0, 10.0, 12.0]
ref_tau = [100, 30, 15, 4, 2, 0.8, 0.5]  # approximate from BTE literature
...
ax3.set_ylabel(r"$-\mathrm{Tr}[\mathrm{Im}\,\Sigma^R]/n_{dof}$ (rad/s)$^2$")
...
# Bose-Einstein expectation: n(n+1) ~ (kT/hw)^2 for high T
# But for a distribution of frequencies, it's approximately linear
...
# (f) Comparison with Guo Fig 5(b) — ballistic Si/Ge interface
...
# Try to load our ballistic Si/Ge interface result
ifc_file = work_dir / "si_ge_interface_fc_nn.npz"
if ifc_file.exists():
...
plt.tight_layout()
fig.savefig(work_dir / "anharmonic_comparison.png", dpi=150)
plt.close("all")
