...
def _gamma_project_M_blocks(
    ...
    equal to 1), so the einsum reduces to summing over supercell-image
    rows/cols grouped by ``prim_indices``. We do that fold directly with
    two ``np.add.at`` reductions, avoiding the ``(n_dof, dim_sc)`` dense
    T0 (~99 % zeros at Γ-only).

    Bit-for-bit identical to the einsum path; pinned by
    ``test_anharmonic.test_gamma_projection_matches_einsum``.
    """
    ...
    # Fold axis 1 (i = i_sc * 3 + i_cart) onto (c_atom, c_cart) by summing
    # all i_sc with prim_indices[i_sc] == c_atom into c_atom.
...
def anharmonic_transmission_finite(
    ...
    # Phi at Gamma. The gathering matrix at q=(0,0) is real and integer-
    # valued (T0[c, i] = 1 iff prim_indices[i//3]==c//3 and i%3==c%3, else 0),
    # so the einsum 'ci,aij,dj->acd' is just a "fold supercell-image rows
    # and columns onto the primitive-cell index". We do that by direct
    # reduction; the explicit T0 (n_dof × dim_sc, ~99 % zeros) is skipped.
    M_blocks = M_stacked.reshape(n_dof, dim_sc, dim_sc)
    Phi = _gamma_project_M_blocks(M_blocks, prim_indices, n_atoms)
