...
def write_fc3_blocks(
    ...
    *,
    ...
    """Write a block-sparse FC3 to ``fc3_blocks.hdf5``.

    Schema::

        /fc3_blocks/I_J_K               complex (b_I, b_J, b_K)
        /fc3_blocks/I_J_K.attrs         {I, J, K, b_I, b_J, b_K}
        /meta/block_sizes               int64 (n_blocks,)
        /meta/units                     str ("THz^2")
        /meta/keys                      int64 (n_keys, 3)  -- index list

    Group keys use ``"_"`` separators (HDF5 disallows ``"/"`` in path
    components and ``","`` is awkward in pandas-style readers).
    """
    import h5py
...
def read_fc3_blocks(
    path: Path,
) -> tuple[dict[tuple[int, int, int], np.ndarray], np.ndarray, str]:
    """Read the block-sparse FC3 file written by :func:`write_fc3_blocks`.

    Returns
    -------
    phi_blocks
        ``{(I, J, K): ndarray (b_I, b_J, b_K)}``.
    block_sizes
        ``int64 (n_blocks,)``.
    units
        Unit string from the file (e.g. ``"THz^2"``).
    """
    import h5py

    with h5py.File(str(path), "r") as f:
        block_sizes = np.asarray(f["meta/block_sizes"], dtype=np.int64)
        units = str(f["meta"].attrs.get("units", ""))
        grp = f["fc3_blocks"]
        phi_blocks: dict[tuple[int, int, int], np.ndarray] = {}
    ...
    return phi_blocks, block_sizes, units
