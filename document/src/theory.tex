...
\begin{equation}\label{eq:phph_pair_index}
  \mathcal{P}(I,J)
  = \bigl\{(K_1,K_2,K_1',K_2') :
      \Phi^{(3)}_{I,K_1,K_2},\;
      \Phi^{(3)}_{J,K_2',K_1'}\;\text{present},\;
      g_{K_1K_1'},\,g_{K_2K_2'}\;\text{in band}\bigr\},
\end{equation}
encodes which inner block-quadruples contribute to a given output
block-pair $(I,J)$: the ring carries the \emph{full off-diagonal}
propagators $g_{K_1K_1'},g_{K_2K_2'}$ (with $|K_i-K_i'|\le r_c$), not
only the diagonal $g_{K_iK_i}$, so the inter-slab coherence is
retained. From \cref{eq:fc3_block_support}, the $G$-band, and the
output BT-band ($|I-J|\le 1$), the cardinality is bounded by
$|\mathcal{P}(I,J)| = \cO(N_\mathrm{c,x}^{2}\,r_c^{2})$; the pair-index
gate $\Phi^{(3)}_{J,K_2',K_1'}$ exist ties $K_i'$ to the
nearest-neighbour shell of $J$, keeping the count $\cO(1)$ at
$N_\mathrm{c,x},r_c=\cO(1)$ (a few tens of quadruples for a cubic
lattice with $N_\mathrm{c,x}=3$).
...
$|\mathcal{P}(I,J)|=\cO(N_\mathrm{c,x}^{2}r_c^{2})$ (the $r_c^{2}$ from
the full off-diagonal ring, $\cO(1)$ at $r_c=\cO(1)$), the total
q-space SSE cost reduces to
