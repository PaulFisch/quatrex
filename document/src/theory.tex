...
the full $\omega$-axis for its share of the BT-band non-zeros. With
$\mathrm{nnz}(G)$ per $\omega$ given by
\cref{eq:G_bb_nnz} times the intra-block density $C_a/\NBS$, the
per-rank footprint is
\begin{equation}
  M^{(1)}_\mathrm{rank}
  = \cO\!\Bigl(\frac{N_\omega \cdot N_{AO}\,r_c\,C_a}{P}\Bigr).
\end{equation}
Computation: each non-zero $(i,j)$ entry is FFT'd independently
along $\omega$; per-rank work is the same expression times
$\log N_\omega$:
\begin{equation}
  W^{(1)}_\mathrm{rank}
  = \cO\!\Bigl(\frac{N_\omega\log N_\omega \cdot N_{AO}\,r_c\,C_a}{P}\Bigr).
\end{equation}
...
\cref{eq:cic_sparse}; summed over all working points and
distributed across all ranks,
\begin{equation}
  W^{(2)}_\mathrm{rank}
  = \cO\!\Bigl(\frac{N_q^{2}\,N_\tau\,N_{AO}\,N_\mathrm{c,x}^{2}\,C_a^{3}}{P}\Bigr).
\end{equation}
...
\begin{equation}
  M^{(2)}_\mathrm{rank}
  = \cO\!\Bigl(\frac{N_{AO}\,N_\mathrm{c,x}^{2}\,C_a^{2}}{P_S}\Bigr).
\end{equation}
...
$\Sigma$ inherits the BT band of $\Sigma$ and the intra-block
atom-pair sparsity inherited from the $g$ blocks (the contraction
with $\Phi$ does not introduce new non-zeros beyond the FC3
support). The output non-zero count per $\omega$ is therefore
$\cO(N_{AO}\,C_a)$, similar to $G$ but without the BT-band factor
$r_c$. Memory, computation, and communication scale as
\begin{equation}
  M^{(3)}_\mathrm{rank}
    = \cO\!\Bigl(\frac{N_\omega\,N_{AO}\,C_a}{P}\Bigr),
  \qquad
  W^{(3)}_\mathrm{rank}
    = \cO\!\Bigl(\frac{N_\omega\log N_\omega\,N_{AO}\,C_a}{P}\Bigr).
\end{equation}
...
halo of width $N_\mathrm{c,x}$. The halo traffic per
$(P_E,P_S)$ rank is
\begin{equation}\label{eq:halo_volume}
  V_\mathrm{halo}^\mathrm{rank}
  = \cO\!\Bigl(\frac{N_\tau\,N_\mathrm{c,x}\,\NBS\,C_a}{P_E}\Bigr),
\end{equation}
independent of $\NB$.
...
\begin{align}
  ...
  V^\mathrm{rank}_\mathrm{phph}
    &= \cO\!\Bigl(\frac{N_\omega\,N_{AO}\,r_c\,C_a}{P}\Bigr),
...
$A$ or $B$ in \cref{eq:phph_step_A,eq:phph_step_B}, of nominal size
$\cO(N_\mathrm{fft}\,\NBS\,C_a^{2})$ on the joint sparse support of
$\Phi_L$ and $g_b$. At production block sizes
($\NBS\sim 2\times 10^{3}$, $C_a\sim 30$, $N_\omega\sim 10^{4}$),
this is $\cO(\mathrm{few}\;\mathrm{GB})$ per block triplet in
complex FP64. The $\tau$-axis is point-wise within Stage~2, so it
can be batched without inducing additional communication; batches
are sized adaptively from the free HBM at runtime, and the
corresponding range of $w$-indices is processed before moving to the
next batch. The output $(I,J)$-axis can be batched analogously,
since the contributions from different $(I,J)$ pairs at fixed
$\tau$ are independent.

The FC3 dictionary, with non-zero count
$\cO(\NB\,N_\mathrm{c,x}^{2}\,\NBS\,C_a^{2})$ after intra-block
sparsity \cref{eq:phph_intra_block_nnz}, is partitioned along its
first leg $I$ across the $P_S$ spatial ranks. With $N_a\sim 700$
atoms per cell ($\NBS\sim 2000$), $N_\mathrm{c,x}=3$ shells, and
$C_a\sim 30$ neighbours within cutoff, the per-rank FC3 footprint
becomes a few GB and fits comfortably in HBM. If even smaller
footprints are required, the compressed FC3 representations of
\cref{eq:fc3_lowrank,sub:fc3_compression} replace the per-block
$\NBS\,C_a^{2}$ count by $R\,\NBS$ at moderate $R$ and integrate
into the same kernel and distribution scheme without further
modification.
...
Within each $\NBS\times\NBS$ block, $g_{IJ}$ has density $C_a/\NBS$
(\cref{eq:phph_intra_block_nnz}). The block-level SpMM $V_I=\sum_J
g_{IJ}\,W_J$ is therefore a sparse--dense block product with cost
...
\begin{enumerate}
  ...
  \item \emph{Step \cref{eq:kernel_step4}: sparse-output sandwich.}
        ...
        and intra-block atom-pair sparsity (\cref{eq:phph_intra_block_nnz}).
