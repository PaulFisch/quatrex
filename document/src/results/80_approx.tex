...
% REVIEW: eta enters as i*eta added to omega^2, so its unit is THz^2 not THz
$0.79\to0.92$ across $\eta=0.30$--$0.90$\,THz$^2$; extrapolating the converging window
% REVIEW: reconcile with sec:cons_ratio directly-converged eta=1e-12 fixed point 0.57 (~40%)
to $\eta\to0$ gives $\approx0.57$, a $\sim40\%$ reduction (\cref{sec:cons_ratio}) -- roughly twice the
% REVIEW: eta given as the THz^2 value (i*eta added to omega^2)
$\sim16\%$ read off at the single point $\eta=0.45$\,THz$^2$. A single-$\eta$ number
