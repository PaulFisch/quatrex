...
$-2.94\to-2.98$), because the tapered $\Sigma_s$ is no longer the
$\Phi$-derivable functional of the \emph{actual} $G$ below
$\omega_{\rm reg}$ --- the interaction down-converts current spectrally
(the interior bond current \emph{exceeds} the leads below $\sim10$~THz and
deficits above), and the taper's modified infrared occupancy leaves part of
that redistribution unaccounted in $\mathrm{Tr}_k$. The $S_3$/fold symmetry
still telescopes the total to zero ($\sum_kP_{\rm abs}=10^{-12}$, lead
balance $6\times10^{-11}$), so all lead-based observables and gates are
untouched; only the \emph{interpretation} of the interior profile on
tapered runs mixes physics with the regulariser.
