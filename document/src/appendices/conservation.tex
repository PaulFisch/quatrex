...
\paragraph{The local (bond) current and the Meir--Wingreen current.}
The per-interface current array holds the same structural form
$\mathrm{Tr}[\Sigma^>G^<-G^>\Sigma^<]$ at every entry: the first and last
entries insert the lead (OBC) self-energy --- the Meir--Wingreen contact
current --- while the interior entries insert the dynamical-matrix embedding
self-energy, the operational form of the Hardy bond current. The two are
related by steady-state energy continuity per slab,
$J_{k+1}=J_k+P_{\rm abs}(k)+D_k(\eta)$, with the \emph{slab-resolved}
scattering absorption $P_{\rm abs}(k)=\sum_\omega\hbar\omega\,
\mathrm{Tr}_k[\Sigma_s^>G^<-\Sigma_s^<G^>]$ (the block-resolved bubble
balance; its device sum is the $J_s$ of \cref{tab:cons_cnt33}, machine-zero
for the conserving vertex --- binned and unbinned agree to $8\times10^{-16}$)
and $D_k$ the finite-$\eta$ ghost absorption. \Cref{fig:cons_local_mw}
verifies the identity on the three-cell CNT: at $\eta=10^{-12}$ the
reconstruction $J_0+\sum_{j<k}P_{\rm abs}(j)$ reproduces the measured
interior bond currents to the SCBA residual level of each run
($2\times10^{-4}$ on the two-cell device, $3\times10^{-3}$ at three cells)
--- the interior dip below the leads is \emph{not} broadening but the
energy the three-phonon interaction extracts near one contact and re-injects
near the other ($P_{\rm abs}=(-3.13,+0.00,+3.13)$), flowing through the
interaction channel that the harmonic bond current does not count. At
$\eta=0.7$~THz the same scattering term explains only $\sim9\%$ of the dip;
the remaining gap \emph{is} the slab-resolved $\eta$-ghost absorption
($D_k\approx-1.76$ per contact slab), which cancels left--right so the lead
balance survives. One cannot ``sum'' local currents into the Meir--Wingreen
current --- in steady state every interface would carry the \emph{same}
current if energy flowed only through the harmonic bonds; the telescoped
absorption is exactly what reconstructs the lead values from the interior.

A caveat isolated by an L2 recipe ablation: the \emph{infrared occupancy
taper} (\cref{app:infrared}) breaks this \emph{interior} bookkeeping
while leaving every global gate intact. With the taper the identity misses
by $\approx2.9$ ($5\%$ of $J$; the smooth spectral window adds nothing,
$-2.94\to-2.98$). Mechanistically the taper multiplies the \emph{contact
injection occupancies} by $\omega^2/(\omega^2+\omega_{\rm reg}^2)$
(\texttt{solver.py}) --- a Lorentzian-tailed deformation of the lead
boundary condition that still suppresses scattering by $10\%$ at
$3\,\omega_{\rm reg}$ --- so the tapered device answers a modified-contact
problem whose interior current redistribution (the interaction
down-converts: the interior bond current \emph{exceeds} the leads below
$\sim10$~THz and deficits above) is not fully accounted by
$\mathrm{Tr}_k[\Sigma_s^\lessgtr G^\gtrless]$; the residual attribution of
the missing $2.9$ is under investigation. The $S_3$/fold symmetry still
telescopes the total to zero ($\sum_kP_{\rm abs}=10^{-12}$, lead balance
$6\times10^{-11}$), so all lead-based gates are untouched --- but the
untapered (bare) bubble is the only variant that satisfies the
per-interface ledger, and the bare-vs-tapered conductance difference
($13.6$ vs $17.0$ at $181$ points) is resolved by the bare grid-refinement
ladder, not by the taper family's own $\omega_{\rm reg}\to0$
extrapolation.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{transport_sweeps/local_vs_mw_current.pdf}
  \caption{Per-interface heat current of the three-cell (3,3) CNT (points;
    interfaces $0$ and $3$ are the lead Meir--Wingreen currents, stars)
    versus the energy-continuity reconstruction from the slab-resolved
    scattering absorption (line). \emph{Left}: at the $\eta=10^{-12}$
    conserving fixed point the reconstruction matches the interior bond
    currents --- the dip is interaction-channel energy transport.
    \emph{Right}: at $\eta=0.7$~THz the gap between reconstruction and
    measurement is the slab-resolved finite-$\eta$ ghost absorption.}
  \label{fig:cons_local_mw}
\end{figure}
...
systems (\texttt{d5a}) quote the directly converged $\eta\to0$ conductance
(method-invariant at the one-percent level, $G=2.61$--$2.64$), noting the residual
$0.016$--$0.032$ self-energy floor and the grid-consistent ghost-reservoir lead
term, rather than an $\eta$-floor-limited extrapolation (\cref{sec:res_eta0}).
