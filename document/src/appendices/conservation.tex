...
(\cref{tab:cons_toy}). This confirms that the half rule and the
self-energy cutoff are harmless to conservation.
...
the bare \emph{lead} balance is $|J_L+J_R|=2.9\times10^{-1}$, carried entirely
...
removes both the floor ($\propto\eta$) and the conductance systematic, limited
only by SCBA stability for soft modes (\texttt{d5a} does not converge below
$\eta\approx0.11$). \emph{(3)~A B\"uttiker dephasing probe} on the $\eta$
...
give an $\eta$-compensated balance that does go to zero. For soft systems that
cannot lower $\eta$ (\texttt{d5a}), quote the $\eta$-floor-limited number with
its band and the marginal-quasiparticle caveat ($\gamma/\omega\sim0.5$).
