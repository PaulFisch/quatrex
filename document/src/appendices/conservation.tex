...
% REVIEW: FLAG coverage gap -- the sigma_convention positivity gate (-i Sigma^{><} >= 0) is named here but never exercised/quantified in the body
...
\begin{table}[t]
  ...
  \begin{tabular}{llr}
    ...
    % REVIEW: reordered case rows to S1..S5 (S3 was listed after S4)
...
% REVIEW: lead balance must equal |J_s|=0.306 by J_L+J_R+J_s~=0; 2.9e-1 was ~5% off
...
the retired acoustic-sum-rule projection broke $S_3$ at $O(1)$ --- confirming
% REVIEW: FLAG over-generalisation -- a permutation-symmetric ASR projection preserves S3; the O(1) breaking is specific to the non-symmetric projection used here
that the vertex must be left as a genuine third derivative
(\cref{eq:H_phonon}) rather than post-projected.
...
% REVIEW: FLAG inconsistent statement -- eta-window extrapolation (0.68-0.74) vs directly-converged 0.57 differ by ~13 pts (as large as the systematic removed); "13-22 point" mixes two undefined figures, make consistent
...
$\eta\ge0.30$ window to $\eta\to0$ (quadratic $0.68$, linear $0.70\text{--}0.74$)
already gives $G_{\rm anh}/G_{\rm ball}\approx0.70$, a $\sim30\%$ reduction ---
\emph{double} the $\sim16\%$ read off at the single point $\eta=0.45$. The
directly converged $\eta=10^{-12}$ fixed point of \cref{sec:res_eta0} lies lower
still, at $0.57$: the ratio keeps falling below $\eta=0.30$ faster than the
window fit, so the extrapolation is only a lower bound on the effect and the
physical reduction is $\sim40\%$. That $\eta=10^{-12}$ value is trustworthy
here, not an under-resolved artefact --- it conserves at $7\times10^{-6}$ (lead
balance), pins the bubble balance at $10^{-16}$ every iteration
(\cref{fig:res_eta0_conv}), and shifts by only a few percent under the infrared
regulariser and grid refinement (\cref{fig:res_eta0_cutoff}), far less than the
$\sim13$--$22$-point $\eta$ systematic. Refining the grid at fixed $\eta=0.45$
