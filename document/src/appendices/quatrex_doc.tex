...
periodic transverse directions, performs the transverse Fourier sum
given in \cref{eq:assemble}.
...
\begin{tabular}{lll}
  ...
  \texttt{phonon.eta}                 & \texttt{0.45}           & broadening $\eta$ (THz), cf.\ \cref{eq:regulator_impl} \\
...
\end{lstlisting}
The result is a single square sparse matrix of size
$(N_\mathrm{blocks}B)\times(N_\mathrm{blocks}B)$, where $B$ is the block size and
...
Throughout the device builder, operators are stored and assembled as a
single upper-triangular half: \texttt{load\_matrices} keeps only the
\texttt{triu} of each loaded block, and \texttt{\_construct\_transport\_cell}
reconstructs each full block as
\texttt{block + triu(block\_flipped, k=1).conj().T}, where
\texttt{block\_flipped} is the block at the negated coordinate, so that
Hermiticity is enforced by construction and the strictly-lower part is
never stored redundantly.  When several blocks or $k$-points are combined
(\texttt{assemble\_matrix}, \texttt{\_sum\_operator}), a common sparsity
pattern is obtained as the union of the per-shift patterns (symmetrised
via $S + S^\mathrm{T}$) so that no entry present in any contribution is
dropped.
...
couplings on either side of the interface.  The boundary blocks are taken
\emph{literally} from the lead dictionaries (both sub- and super-diagonal,
never as conjugate transposes of one another, since at non-zero transverse
shift $H_{10}(\vc{R}_\perp)=H_{01}(-\vc{R}_\perp)^\dagger$, not
$H_{01}(\vc{R}_\perp)^\dagger$); Hermiticity of the assembled matrix
therefore \emph{presupposes} that the supplied lead blocks already satisfy
\cref{eq:hermiticity_perp} --- the assembly does not re-symmetrise them.
