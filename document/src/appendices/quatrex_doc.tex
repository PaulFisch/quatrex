...
% REVIEW: code/string literal does not belong in a numbered equation; use inline \texttt
form \texttt{"[nx, ny, nz]"},
...
% REVIEW: removed duplicate of eq:assemble; reference the single definition instead
...
\end{lstlisting}
% REVIEW: ambiguous notation; intended is a square matrix of linear size N_blocks*B (i.e. (N_blocks*B) x (N_blocks*B))
...
% REVIEW: document the upper-triangular-half Hermitian-operator storage/assembly convention used by the device builder
...
% REVIEW: flag -- consistency assumes the supplied lead blocks already satisfy H_10 = H_01^dagger; state this prerequisite
