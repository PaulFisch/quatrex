...
\cref{sec:finite_validation}: FC2/FC3 quality, sparsity structure,
...
\cref{ap:conventions_units}).  Force constants are mass-weighted up
...
\item[\texttt{sse\_cutoffs.py}] Three-phonon bubble in block-tridiagonal
  ...
  True}.  Five cutoff knobs documented in \cref{sec:fv_cutoffs}.  The
...
\item[\texttt{physical\_tests.py}] Static FC2/FC3 invariants
  ...
  bulk-Si reap is structural rather than a fit defect
  (\cref{sec:leg_j_rel}).  Downstream consumers that need strict
...
\item[\texttt{decomposition.py}] Drives the six existing FC3 tensor
  ...
  \cref{sec:fc3_compression}) over a rank ladder, reporting parameter
...
\subsection{Bare propagator}
\label{ap:fa_obs_bare}

In the THz$^2$ unit system, the bare retarded propagator is
\begin{equation}
...
\begin{equation}
  ...
  \Sigma^{<,>}_{IJ}(\omega) = \frac{i\hbar}{2}
    ...
    \Phi^{(J, K_2', K_1')},
\end{equation}
with the inner sum implicitly an $\omega$ convolution.
...
\begin{enumerate}
  \item Run the cluster pipeline as described in
    \cref{sec:fv_systems} and \cref{sec:input_gen}; the reap lands at
