...
\item[\texttt{constants.py}] Single home for every numeric default in
  ...
  $\texttt{DEFAULT\_ETA\_THZ\_TRANSPORT} = 0.05$~THz (the transport-path
  broadening of \cref{eq:gf_retarded_bare});
...
\item[\texttt{physical\_tests.py}] Static FC2/FC3 invariants
  ...
  a residual reading of $\sim 0.8$ on a SiNW reap ($0.796$ measured on
  the \emph{device-truncated} d5a tensor, whose leg sums run over the
  kept atoms only) and $\sim 0$ on a bulk-Si reap is structural rather
  than a fit defect.  Downstream
...
Three $\eta$ conventions coexist in this report and should not be
confused.  The dense reference (this package and
\cref{sec:phonon_solver_pkg}) damps through $(\omega+i\eta)^2$ with
$\eta$ in THz, set by the \texttt{eta\_factor} conventions of
\cref{sec:solver_units_grid} (linear in $\Delta\omega$); the
production solver uses the frequency-proportional regulator
$\omega^2+2i\eta|\omega|$ of \cref{eq:regulator_impl}, also with
$\eta$ in THz; and the additive $i\eta$ of the theory-chapter Dyson
equation \cref{eq:system} is an idealisation whose $\eta$ carries
THz$^2$.  All quoted broadenings in this document are in THz unless
the $\omega^2$-additive form is written explicitly.
...
(\cref{eq:bosonic_fold}) and are anti-Hermitian on the DOF axes;
...
% REVIEW(verified): reconciled to the ~0.8 of the module reference -- 0.796 was measured on the device-truncated d5a tensor (conserving_vertex_findings.md); the raw hiphive fits are ASR-exact (residual ~1e-14), so no "hiphive least-squares gave 60%" run exists
  (\texttt{fc3\_asr\_legs}).  When violated (e.g.\ the
  \emph{device-truncated} SiNW tensor reads $\sim0.8$; the raw hiphive
  fits are ASR-exact), \texttt{load\_quatrex\_blocks(asr\_project=True)}
