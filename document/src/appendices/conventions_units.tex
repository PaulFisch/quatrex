...
\begin{equation}\label{eq:VaspToTHz}
  ...
  = \mathrm{sgn}(\lambda)\,\sqrt{|\lambda|}
    \times f_0,
  \qquad
  f_0 = \frac{1}{2\pi}\sqrt{\frac{e}{m_u A^2}} \approx 15.633\;\mathrm{THz},
\end{equation}
...
% REVIEW(open): theory/30_scba_eta0.tex (l.310) still writes G^<_eq=-inA without noting it is the textbook sign, opposite to the stored occupation-positive solver sign (+inA); infrared.tex now states this explicitly, the theory file is outside this appendix set
