...
\texttt{CONVERSION\_THZ2 = e/(m_u A^2)/(2\pi)^2 \cdot 10^{-24}} converts
...
squares; \cite{Togo2015symfc} for the underlying algorithm), and
...
\texttt{d3\_qq2rr.x} for FC3 \cite{Mounet2005phonons}.  The sow stage
...
\begin{description}
  \item[\texttt{sow}.]  Generates rattled supercells.  For the default
        ...
        per-atom displacement scales linearly in \texttt{rattle\_n\_iter}
        (each MC step adds an independent Gaussian increment).  For
...
\begin{description}
  ...
  \item[\texttt{PCP}.]  The permanent-CP ansatz of
        \cite{eriksson_hiphive_2019}-cited Luo et al.\ (2025), wrapped
...
\texttt{finite\_analysis} (\cref{sec:transport_quality}) reads on
restart.
...
throughout the rest of this report.  The investigation began when, even
...
\cite{Tadano2015tdep}, but for an H-passivated SiNW at its ground-state
...
\begin{table}[ht]
  ...
  $\omega$ on the supercell q-mesh.  The Γ-point three-acoustic-mode
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.16\linewidth} p{0.42\linewidth} p{0.34\linewidth}}
    ...
    FC2 cutoff $<$ wire diameter
      & d5a sc4: 7 Γ-imag modes ($\omega \in [-2.2,-1.2]$ THz), 90 \%
        Si-localised.
    ...
    Over-aggressive mc-rattle on light H atoms
      & d9a sc4 with $n_\mathrm{iter}=10$: rattle peaks 0.27–0.37 Å,
        ...
        (51 Γ-imag modes at –14 THz, 91 \% H-localised).
  ...
  \cite{Rurali2010colloquium,Peelaers2009sinw,CartoixaRurali2010}.}
...
\emph{introduces} imaginary modes, a trade-off discussed in the
context of layered systems by \cite{Mounet2005phonons}.
...
\begin{equation}\label{eq:suggested_cutoff}
  r_c^{(2)} = \min\bigl(12.0,\ \max(5.0,\ 2\rho + 0.5)\bigr) \quad [\mathrm{\AA}],
\end{equation}
...
recommended range only at first glance.  In hiphive's mc-rattle the
per-atom cumulative displacement scales \emph{linearly} in
$n_\mathrm{iter}$ -- each Monte-Carlo step adds an independent
Gaussian increment of standard deviation $\mathrm{rattle\_std}$ -- so
the realised displacements after $n_\mathrm{iter}=10$ are typically
$\mathrm{rattle\_std}\times n_\mathrm{iter} \approx 0.3$ Å, not the
$\mathrm{rattle\_std}\sqrt{n_\mathrm{iter}} \approx 0.1$ Å a
square-root scaling would suggest.
...
at $\vc{q} \neq 0$ silently used wrong phases, scrambling the Γ-to-Z
...
cone, \emph{adding} Γ-imaginary modes.  At cutoffs that span the wire
...
\begin{itemize}
  \item Peelaers et al.\ \cite{Peelaers2009sinw} compute ab-initio
        ...
        d12a structures.
  \item Cartoixà and Rurali \cite{CartoixaRurali2010,Rurali2010colloquium}
        enumerate the canonical $\langle 100 \rangle$ H-passivated SiNW
        family Si$_9$H$_{12}$, Si$_{21}$H$_{20}$, Si$_{37}$H$_{28}$, $\ldots$
...
as TODOs and will be filled into \cref{sec:fv_quality_sinw} once the
