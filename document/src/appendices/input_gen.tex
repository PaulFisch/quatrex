...
cell and avoids supercell calculations. However, QE only supports a restricted set of
...
$\approx 0.80$ appears for every SiNW hiphive reap processed by the
pipeline and is structural, not a fit defect: it is a property of the
truncation to the open device, not of the fit itself.  The metric
measures
\begin{equation}
  \mathrm{leg\_j\_rel} =
...
projection on the FC tensor itself.  In practice the raw
(periodic-supercell) hiphive fits nevertheless come out ASR-exact on
all legs (residual $\sim10^{-14}$); the finite reading arises once the
tensor is truncated to the open device, whose leg sums then run over
the kept atoms only (\cref{ap:fa_validation}).
...
mirror symmetry yield $\approx 0.80$ on the device-truncated tensor
($0.796$ measured for \texttt{d5a}) regardless of
$\mathrm{fit\_method}$, $n_\mathrm{structures}$, or cutoffs.  The
metric is therefore a structural feature of the open-device truncation
of a low-symmetry quasi-1D system, not a fit defect.
