...
\begin{align}
  ...
  \Phi_3\;[\mathrm{eV}/\text{\AA}^3]
    ...
    % REVIEW: E_h/2/a_0^3 = 91.82, not 91.84 (rounding/typo)
...
% REVIEW: removed verbatim duplicate of eq:fc2_conv/eq:fc3_conv; reference instead
...
% REVIEW: symbol 'A' is overloaded -- lattice-vector matrix in eq:q_conversion
% vs the Angstrom length unit in C=e/(m_u A^2) below / eq:conversion_fc3_ifc.
...
% REVIEW: was \texttt{CONVERSION_THZ2 = ... \cdot 10^{-24}} with math inside
% text-mode \texttt{} (Missing $) and a $^2$$\cdot$ double-dollar; fixed.
...
output mode.
% REVIEW: document the NN block-triplet truncation applied by the device FC3 loader (fc3_loader.py)
...
% REVIEW: re-sourced. Mounet & Marzari 2005 is a harmonic DFPT/quasiharmonic
% study and does not provide the D3Q third-order pipeline; the D3Q/thermal2
% references are paulatto_anharmonic_2013 and fugallo_ab_2013.
...
\begin{description}
  \item[\texttt{sow}.]  Generates rattled supercells.  For the default
        ...
        % REVIEW: independent Gaussian increments -> variances add -> std ~ sqrt(n_iter), not linear
...
\begin{description}
  ...
  \item[\texttt{CP} / \texttt{INDSCAL} / \texttt{Waring}.]
        ...
        lifted target.
  % REVIEW: fixed garbled citation -- PCP ansatz is from luo_tensor_2025, not a 2019-cited 2025 ref
...
% ======================================================================
% REVIEW: restored missing \section heading; label had no preceding section title
...
% REVIEW: original key Tadano2015tdep was unresolved (it conflated Tadano with
% Hellman's TDEP and the SrTiO3-only paper does not cover these examples);
% re-sourced to the SCPH review. Confirm a citation that names these examples.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.16\linewidth} p{0.42\linewidth} p{0.34\linewidth}}
    ...
    Over-aggressive mc-rattle on light H atoms
      % REVIEW: these 0.27-0.37 A peaks are max-over-atoms (extreme-value), consistent with the sqrt(n_iter) law, not linear scaling
      & d9a sc4 with $n_\mathrm{iter}=10$: rattle peaks 0.27–0.37 Å,
...
\begin{table}[ht]
  ...
  % REVIEW: inconsistency -- 7.0 A does NOT exceed the stated 7.4 A full diameter (2*rho); wording needs fixing
  ARDR + post-fit rotational sum rule succeeds only once
  $r_c^{(2)} \geq 7.0$ Å, i.e.\ once the cutoff exceeds the H-envelope
  diameter.  All fits use FC3 cutoff 4.0 Å, $n_\mathrm{structures}=16$.}
...
% REVIEW: original key Mounet2005phonons was unresolvable and is a harmonic-DFPT
% / quasiharmonic paper that does not establish this rotational-sum-rule vs PSD
% trade-off for layered systems; citation dropped -- confirm the intended source.

% REVIEW: d5a FC2 cutoff is inconsistent across the appendix (7.0 here / 7.8 in table / 7.88 in verification / 7.9 in eq:suggested_cutoff); converge on one number
Refitting the d5a sc4 supercell at $r_c^{(2)} = 7.0$ Å with ARDR + RSR
gives a clean dispersion across the full $\Gamma\to Z$ path: no
...
% REVIEW: validator says >= 45 but eq:nstruct_recommendation yields 44 for d5a sc4 (84 atoms); reconcile 44 vs 45
already flags this with severity \texttt{warn} (it raised
``\texttt{n\_structures should be $\geq$ 45 for 84-atom supercell}'' on
...
recommended range only at first glance.  % REVIEW: a sum of n independent zero-mean Gaussians has std = sigma*sqrt(n) (variances add), not sigma*n; corrected to sqrt scaling
...
\begin{itemize}
  \item Peelaers et al.\ \cite{Peelaers2009sinw} compute ab-initio
        ...
        d12a structures.
  % REVIEW: original key CartoixaRurali2010 was unresolvable (no two-author
  % Cartoixa & Rurali 2010 paper enumerating this family was found); confirm
  % the intended source for the Si9H12/Si21H20/Si37H28 enumeration.
