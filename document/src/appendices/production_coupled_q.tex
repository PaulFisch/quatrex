...
verification, not new physics.
% REVIEW(open): the numerical record of the 2026-06-08 session (phonon/scripts/out/production_coupled_q/ and phonon/scripts/verify/production_coupled_q_summary.py) was purged in the studies reorganisation (commit 843c3069); the film figures below survive from that session, the CNT ladder data does not
The figures below are the surviving artifacts of the original
(2026-06-08) verification session; the consolidation script and run
snapshots were removed in a later cleanup of superseded one-off
scripts.
...
projection; the dense solver at the matched coarse mesh and broadening is
comparably non-conserving ($17\%$, \cref{tab:prod_dense}). At small
broadening on the converged mesh the dense solver reaches $\sim10^{-5}$
on the same physical system (dashed); that four-decade gap is the open
issue of \cref{sec:prod_open}.}
...
% REVIEW(open): source run purged -- the L=2,4,6 eta=0.45 ladder snapshots were deleted with phonon/scripts/out/production_coupled_q/ (commit 843c3069), and the current phonon/scripts/out/prod/cnt33/summary.csv is a superseded, unconverged finite-eta run (L2/L3/L4 ratios 1.32/1.80/1.89, L3/L4 not converged), so the 0.896/0.911 best-iterate ratios are no longer reproducible; superseded by the eta=0 ladder in sec:res_eta0. fig:prod_cnt_ladder removed (the regenerated PDF plots the superseded run, not the session described here).
...
cells at $\eta=0.45$\,THz on a $121$-point grid with $50$ iterations. The
anharmonic self-consistency on this soft-twist tube is metastable at this
large broadening: the iteration does not settle to a fixed point but
passes through a conserving iterate that the heat-flow criterion captures
(the final iterate diverges and is discarded). The best-iterate ratios of
that session ($G_{\rm anh}/G_{\rm ball}\approx0.90$ at $1$--$3.5\%$
lead conservation) were weaker (closer to one) than the dense
small-$\eta$ ladder of \cref{sec:res_cnt}, consistent with the
under-scattering discussed in \cref{sec:prod_open}. The run snapshots
have since been purged, so those ratios are quoted here for the
historical record only; the definitive production CNT length ladder is
the directly converged $\eta\to0$ one of \cref{sec:res_eta0}, and the
surviving coupled-$q$ verification record is the film sweep of
\cref{fig:prod_qfilm_qconv,fig:prod_qfilm_conservation}.
...
$\eta\leq0.2$ the Dyson solve
$[(\omega^2 + 2i\eta|\omega|)\,I - D - \Sigma^R]$ (the production
frequency-proportional regulator of \cref{eq:regulator_impl}, $\eta$ in THz)
becomes singular and the block recursion returns NaN, where the dense solver's
