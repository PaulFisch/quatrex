...
solver and every run made to validate and exercise it during the
2026-06-08 session. The transversely-periodic physics and the silicon
thin-film benchmark were already established in the standalone (dense)
solver (\cref{sec:res_qfilm}); the contribution here is software and
...
\texttt{phonon/scripts/verify/production\_coupled\_q\_summary.py}. Results
that are still running at the time of writing are marked as such; nothing
below is extrapolated.
...
\begin{table}[h]
\centering
\caption{Bit-exact verification of the coupled-$q$ port (this session).}
...
conservation $1.0\%$) and $0.911$ ($L{=}4$, conservation $3.5\%$); $L{=}6$
was still running at the time of writing. The large-broadening value is
...
broadening artefact, \cref{sec:res_negative}); only at small broadening
...
exhausted without success in this session: retarded method \texttt{fft},
