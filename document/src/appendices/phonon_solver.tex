...
the driver-level default \texttt{eta\_factor}$=1.0$ in
\texttt{dense.py} gives $\eta\sim\Delta\omega$, while the bare
\texttt{grids.build\_frequency\_grid} signature retains the legacy
$\texttt{eta\_factor}=0.05$), but can be overridden
...
% REVIEW(verified): the kernel does NOT round to a power of two (no rounding anywhere in phonon/solver); n_fft is caller-supplied and se_finite.py:319 / bubble.py:319 pass exactly 2*n_freq-1
FFT length \texttt{n\_fft} is supplied by the caller; the drivers pass
the full linear-convolution length $n_\mathrm{fft} = 2\,n_\mathrm{freq} - 1$
(no power-of-two rounding is applied) --
...
\begin{description}
  \item[\texttt{"half"}.]  Drops the principal-value integral entirely
        ...
        broadening, $\Gamma_\Sigma = i(\Sigma^> - \Sigma^<) = -2\,\mathrm{Im}\,\Sigma^R$.
        Used when only the broadening matters
...
matches the eigenmode Green's functions of \cref{eq:gf_lesser_eigenmode}
and the bosonic continuation \cref{eq:bosonic_fold} of the theory
chapter.
...
\begin{description}
  ...
  \item[\texttt{solve\_green\_functions} /
        ...
        Dyson equation
        $G^R(\omega) = [(\omega+i\eta)^2 - D - \Sigma^R_\mathrm{tot}]^{-1}$
...
\section[Gamma-only self-energy driver: \texttt{se\_finite.py}]{$\Gamma$-only self-energy driver: \texttt{se\_finite.py}}
\label{sec:se_finite}
...
accumulates into the output $\Sigma^{<,>}(q_\mathrm{ext}, \omega)$.  The
...
\begin{table}[ht]
  ...
  \begin{tabular}{lrrrr}
    ...
    system & $f_{\max}$ (THz) & $2\omega_{\max}/2\pi$ (THz)
      & $|\Sigma^{R}|_{\max}$ (THz$^{2}$)
      % REVIEW(verified): |Im Sigma^R|/omega is the FWHM (damping form Sigma^R=-i*omega*Gamma gives A(omega) of full width Gamma); HWHM is half of it -- labelled FWHM to remove the factor-2 ambiguity
      & median $|\mathrm{Im}\,\Sigma^{R}|/\omega$ (FWHM, THz) \\
    \hline
  ...
  fake an inflated $\Sigma$ that destabilises SCBA ---
  $\sim3.3\times$ in $|\Sigma^R|_{\max}$ for bulk Si and $\sim24\times$
  for d5a;
...
matrix at $\Gamma$. Added to $D$ in the Dyson denominator this
...
is preserved as a fallback for harnesses that build $D$ directly
...
eigenvalue of $D+\mathrm{Re}\,\Sigma^{R}(\omega)$ across the grid
...
$\min\lambda(D+\mathrm{Re}\,\Sigma^{R})$), a \texttt{result.npz}
...
mode, i.e.\ $\eta_{w}\!\gtrsim\!\sqrt{\,|\min\lambda(D+\mathrm{Re}\,\Sigma^{R})|\,}$\!\!,
