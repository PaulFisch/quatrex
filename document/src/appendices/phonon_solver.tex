...
\begin{enumerate}
  \item It is the canonical reference against which the production
        ...
        test that asserts byte-identical output on a small synthetic
        Green's function.
...
\texttt{CONVERSION\_THZ2 = e/(m_u \AA^2)/(2\pi)^2 \cdot 10^{-24}}
...
\texttt{eta\_factor}$\cdot \Delta\omega$ (the $\eta = 0.05\Delta\omega$
that has been our working value), but can be overridden directly via
\texttt{eta\_w\_thz}.
...
\begin{description}
  \item[\texttt{sancho\_rubio(z2, H\_00, H\_01)}.]  Surface Green's
        ...
        by iterative decimation \cite{Sancho1985SR}.  The fast iteration
...
\section{$\Gamma$-only self-energy driver: \texttt{se\_finite.py}}
\label{sec:se_finite}
...
\section{$q$-resolved self-energy driver: \texttt{se\_q.py}}
\label{sec:se_q}
...
own installed copy at \texttt{quatrex.phonon.bubble}.  The dense
solver of this appendix is the reference implementation: it is the
arbiter of correctness for the production stack.
...
\subsection{A bug found and fixed: the FFT Hilbert transform}

The retarded self-energy is reconstructed from $\Sigma^{<,>}$ by the
Kramers--Kronig relation
$\Sigma^{R}=\tfrac12(\Sigma^{>}-\Sigma^{<})
+\tfrac{i}{2}\mathcal{H}[\Sigma^{>}-\Sigma^{<}]$. The audit found that the
...
$\sim 0.024$\,THz, well within the expected range. For d5a the
...
\begin{table}[h]
  ...
  the bulk-Si linewidth under the proper grid matches the experimental
...
recipe~\cite{walker_anderson_2011,toth_anderson_2015}: history is
...
$\omega\approx 0.028$\,THz, which gives $n_{B}\approx 700$ at room
...
of apparent improvement. The standard cure
\cite{pourfath_causal_2010} is to project $\Sigma^{R}$ onto the
...
\cite{bescond_LOA_pade_2011}. The bubble self-energy scales as
...
\subsection{Joint $\eta\!\to\!0$ and $\lambda\!\to\!1$ convergence diagnostic}
...
\cite{bescond_LOA_pade_2011}.
\end{itemize}
The physical observable lives at $(\eta_{w}\!\to\!0,\lambda\!=\!1)$;
the two natural routes to it from a numerically tractable point are
sketched in figure~\ref{fig:phonon_solver:eta_lambda}, and the
diagnostic content lies in whether they agree.
...
of~\cite{bescond_LOA_pade_2011} runs the SCBA at
...
\cite{bescond_LOA_pade_2011}, and the four-phonon scattering
...
$T=300$\,K the Bose factor $n_{B}\approx 700$ on the twist makes the
...
The bubble is the Migdal form of Guo \emph{et al.}, Phys.\ Rev.\ B
\textbf{102}, 195412 (2020), Eq.~8; the prefactor, the
...
of Guo, Latour and Tian ship in
