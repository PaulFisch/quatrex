...
% REVIEW: 1/(2pi) restored on the LHS to match the eq:g_tau Fourier convention
% (and eq:sigma_phph, which carries d omega'/2pi via the \dw macro). Display only;
% eq:sigma_phph_tau downstream is already correct.
...
from the Si--H stretch needs $f_{\max}\gtrsim 130$\,THz, well above the
...
\begin{equation}\label{eq:cic_cost}
  ...
  = \cO\!\bigl(\ndof^{4}\bigr),
\end{equation}
which we take as the unit cost throughout.
% REVIEW: ndof is overloaded. At the head of this subsection ndof=N_AO=N_B*NBS
% (the full ring), but the block-sparse per-pair kernel below (eq:cic_sparse,
% eq:cost_opt, eq:phph_step_*) works at fixed block pair, where the relevant
% dimension is the PER-BLOCK size NBS. Read ndof->NBS in those per-pair costs. Balanced-tree is
...
\begin{equation}\label{eq:cost_opt}
  \cO\!\bigl(R\,\ndof\,r_c\,C_a + R^{2}\,\ndof\,C_a\bigr),
\end{equation}
to be compared with the sparse ring cost $\cO(\NBS\,C_a^{3})$ of
\cref{eq:cic_sparse} under the assumption that intra-block
...
\begin{equation}\label{eq:cost_phph_impl}
  ...
  + \cO\!\Bigl(
    ...
    \Bigr).
\end{equation}
% REVIEW: ring-work exponent corrected NBS^3 -> NBS^4. The dense per-pair ring
% (eq:inner_contraction; steps A,B each NBS^4) costs NBS^4, so summed over N_B
% outputs and N_c,x^2 inner pairs the work is N_B N_c,x^2 NBS^4/P
% (= N_AO N_c,x^2 NBS^3/P, matching eq:cost_phph_dist). The implemented full
% off-diagonal ring is more work than the retired diagonal design, not less.
