...
SCBA equations: a four-index ring
...
retarded reconstruction of \cref{eq:sigma_r}) retarded
self-energies.
...
convolution $\int \frac{d\omega'}{2\pi}\;G_1(\omega')\,G_2(\omega-\omega')$. We
...
particular, an H-terminated wire with $\omega_{\max}/2\pi\sim 63$\,THz
from the Si--H stretch needs $f_{\max}\gtrsim 126$\,THz, well above the
$\sim 32$\,THz a bulk-Si device ($\omega_{\max}/2\pi\sim 16$\,THz) would require. \Cref{sec:phonon_solver_pkg}
...
\paragraph{Exact bosonic continuation (production).}
The conjugation form $\mathrm{corr}(g^{\gtrless},(g^{\lessgtr})^{*})$ above is
exact only when the propagators are complex-symmetric (equilibrium, or a
reciprocal device). Under a temperature bias $G^{\lessgtr}$ are no longer
symmetric, and the negative-frequency (absorption) leg must be continued with
the full bosonic relation
\begin{equation}\label{eq:bosonic_fold}
  G^{<}_{ij}(\qp,-\omega) = G^{>}_{ji}(-\qp,\omega),
\end{equation}
i.e.\ with a spatial \emph{transpose} ($ij\to ji$) \emph{and} a transverse-momentum
negation ($\qp\to-\qp$), not merely a conjugation. The production kernel
(\texttt{quatrex.phonon.sse\_phonon\_phonon}, \texttt{\_build\_fold\_plan}/%
\texttt{\_fold\_reversed\_legs}) folds the $\omega<0$ contribution onto the
one-sided $\omega\ge0$ grid through \cref{eq:bosonic_fold}, so each
$\Sigma^{\lessgtr}$ is the sum of the forward ring and the
transposed/$\qp$-reversed ring. This retains the inter-block (off-diagonal)
coherence that the equilibrium conjugation shortcut discards and keeps the bubble
exact and conserving away from equilibrium (\cref{app:conservation}).
...
\begin{equation}\label{eq:cic_cost}
  ...
  = \cO\!\bigl(\ndof^{4}\bigr),
\end{equation}
which we take as the unit cost throughout. Note the two index scales in
use: $\ndof=N_{AO}=\NB\NBS$ for the full ring of this subsection, and
the per-block size $\NBS$ for the block-sparse \emph{per-pair} kernel
of \cref{eq:cic_sparse,eq:phph_step_A,eq:phph_step_B,eq:phph_step_S},
which works at a fixed transport-block pair. Balanced-tree is
...
\paragraph{Dense-kernel caveat (forward note).} Layers (a)--(c) are
implemented in the production kernel of \cref{ssec:phph_dist_impl};
the intra-block layer (d) is an \emph{available} optimisation, not an
implemented one: the production kernel stores each block densely,
which corresponds to setting $C_a=\NBS$ in the bounds above. The
comparisons against the decomposed kernels in
\cref{ssec:phph_separable} and \cref{sec:phph_decomp} refer back to
this caveat.
...
\begin{enumerate}
  ...
  \item For each $(r,s,x)$, compute the inner product
    ...
    $(r,s,x)$: $\cO(R^{2}\,\ndof\,C_a)$.
\end{enumerate}
The combined cost per working point (full ring, all block pairs) is
% REVIEW(verified): the two-stage costs above are full-ring (ndof rows, band r_c),
% not per-pair; per-pair comparison target is therefore NB*Nc,x^2*NBS*Ca^3.
\begin{equation}\label{eq:cost_opt}
  \cO\!\bigl(R\,\ndof\,r_c\,C_a + R^{2}\,\ndof\,C_a\bigr),
\end{equation}
to be compared with the per-pair sparse ring cost $\cO(\NBS\,C_a^{3})$ of
\cref{eq:cic_sparse}, i.e.\
$\cO(\NB\,N_\mathrm{c,x}^{2}\,\NBS\,C_a^{3})$ per working point,
under the assumption that intra-block
...
$R\ll C_a^{2}$ the first (SpMM) term is cheaper, and for
$R\lesssim N_\mathrm{c,x}\,C_a$ the quadratic term as well, making the
separable form the cheaper one; the crossover is analysed in
\cref{ssec:phph_decomp_compute}.
...
The FFT-first ordering is therefore the
...
$N_\mathrm{fft}=2N_\omega-1$ and $g_a,g_b$ are the
$\omega\to\tau$ transforms (Stage~1,
\cref{ssec:phph_distribution}) of the corresponding band blocks of
$G^{\gtrless}$. The intermediates $A,B$ are stored only on the joint
...
\begin{equation}\label{eq:cost_phph_impl}
  ...
  = \cO\!\Bigl(
    ...
    \Bigr)
  + \cO\!\Bigl(
      \frac{N_\tau\,\NB\,N_\mathrm{c,x}^{2}\,\NBS^{4}}{P}
    \Bigr).
\end{equation}
...
\begin{table}[htb]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    FFT-first $\times$ halo (\emph{impl.})
      & $\dfrac{N_\tau\,\NB N_\mathrm{c,x}^{2}\NBS^{4}}{P}$
