...
$G^{<}_{\rm eq}=-i\,nA$ (textbook sign; the solver stores the
occupation-positive opposite, \cref{sec:solver_units}) stays finite despite
$n(\omega')\sim1/\omega'$. Out of equilibrium the bosonic fold
...
back exactly via Ewald summation~\cite{gonze_dynamical_1997}. Here
$\xi_g$ is the short-range decay length set by the electronic band gap
of the insulator, distinct from the thermal length $\xi_e$ of
\cref{eq:fc2_metal}. \Cref{eq:fc2_error_metal} is the 1D geometric
form; in $d$ dimensions the shell degeneracy multiplies the summand by
$\sim R_c^{d-1}$, a polynomial correction to the exponential bound.
...
cost is $\mathcal{O}(N_\omega\,N_q\,\ndof^3\,N_{\mathrm{iter}})$
