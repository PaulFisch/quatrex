...
% REVIEW(verified): per transport block row; times NB gives the per-tau
% eq:per_tau_cost form O(R ndof r_c Ca + R^2 ndof) since ndof=NB*NBS.
propagator-side cost is $\cO(R_2\,\NBS\,r_c\,C_a + R_2^{2}\NBS)$ per
transport block row --- equivalently
$\cO(R_2\,\ndof\,r_c\,C_a + R_2^{2}\,\ndof)$ per $\tau$
(\cref{eq:per_tau_cost}) --- identical in form to the CP family with
$R\to R_2$.
...
\begin{enumerate}
  ...
  \item \emph{Step \cref{eq:kernel_step2}: small dense GEMM.}
        ...
        % REVIEW(verified): GEMM roofline (m=n=R, k=ndof): 2R^2 ndof flops /
        % (2R ndof + R^2) words ~ R flops/word for R << ndof.
        intensity is $\cO(R)$ flops/byte, comfortably
        compute-bound.
...
% REVIEW(verified): the convolution is FFT-able entrywise, but one transverse
% FFT is needed per (r,s) entry and tau-point, not one FFT in total.
The
$\qp'$-sum is therefore a Fourier transform of a per-block product, and
the $N_q^{2}$ factor in \cref{eq:cost_q} is replaced by
$N_q \log N_q$ after FFTs along the transverse axis (one per $(r,s)$
entry and $\tau$-point).
...
memory reduction and the $q$-axis speed-up are reported in
