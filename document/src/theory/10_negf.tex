...
extended to anharmonicity by~\citet{mingo_anharmonic_2006, wang_nonequilibrium_2007},
...
% REVIEW: sign corrected. This file's own eq:spectral gives A=i[G^R-G^A]=-2 Im G^R
% and G^R-G^A=G^>-G^< (relations above), so G^>-G^<=-iA=+2i Im G^R; the FDT must
% therefore read G^{<}=-i n A=+2i n Im G^R (was -2i, internally inconsistent).
...
% REVIEW: the constant i*eta here is the idealised regulator; the production
% solver uses the frequency-proportional form (omega^2 + 2i*eta*|omega|) plus a
% DC-concentrated soft-mode broadening floor on the device matrix only -- see the
% eta->0 discussion (eq:regulator_impl, eq:eta_floor in sub:eta_zero).
\begin{equation}
...
\begin{align}
  H_{00}(\qp)
    &= \frac{C}{N_{q_z}}
       \sum_{m=0}^{N_{q_z}-1}
       D^{(\mathrm{B})}\!\left(q_x,q_y,\frac{m}{N_{q_z}}\right),
  \label{eq:H00}\\
  H_{01}(\qp)
    &= \frac{C}{N_{q_z}}
       \sum_{m=0}^{N_{q_z}-1}
  ...
  \label{eq:H01}
\end{align}
Here $D^{(\mathrm{B})}$ is the bulk mass-weighted dynamical-matrix block and
$C$ is the force-constant-to-$(\text{THz})^2$ conversion constant of
\cref{sec:solver_units} (the sum index $m$ runs over the $q_z$ mesh).
...
\begin{equation}
  ...
  \beta  = -H_{01},
\end{equation}
and iterate (within each sweep all four right-hand sides are evaluated with the
$\epsilon$ from the previous sweep, i.e.\ the updates are simultaneous, not
sequential)
\begin{align}
...
\end{equation}
with $D_{10}=D_{01}^\dagger$ the device--lead coupling block (the off-diagonal of
the full mass-weighted dynamical matrix linking the device edge to the first lead
layer; equal to the $H_{01}$ block when device and lead are the same material).
...
\end{equation}
The relevant decay rates $\lambda_j = e^{i k_j a}$, with $k_j$ the complex band
wavevectors at energy $\omega^2$, are the eigenvalues of the $2N\times2N$
companion linearisation of the quadratic eigenproblem
$(\lambda^2 H_{10}+\lambda(H_{00}-\omega^2\mathbb{I})+H_{01})\xi=0$ (not of the
$N\times N$ operator $T_0$ alone). In a bulk gap,
...
convergence becomes arbitrarily slow (the effective exponential rate
$c_j\eta$ collapses), so many more decimation steps are needed.
