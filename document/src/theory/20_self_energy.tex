...
(i) the four-phonon Hartree--Fock tadpole
$\Sigma^{(4),\mathrm{HF}}$ from $\Phi^{(4)}$ already discussed above
(\cref{fig:fc4_tadpole}), which contributes the
temperature-dependent thermal expansion and frequency
shifts~\cite{wang_nonequilibrium_2014},
...
complexity scales as $\mathcal{O}(N_q^2 N_\omega^2 \ndof^4)$
(the $\ndof^4$, rather than the naive $\ndof^6$ of the
...
independent of $i$ throughout the device, providing a physically meaningful convergence test:
\begin{equation}
...
finite frequency grid). A convergence criterion for SCBA can be
